...
DEFAULT_TEXT_TO_SQL_TMPL = (
    ...
    "To calculate the difference between two Date, always first convert date to format Integer(yyyymmdd). "
    " - Discharge_Date: CAST(strftime('%Y%m%d', Discharge_Date) AS INTEGER) AS date1"
    " - Date_of_Admission: CAST(strftime('%Y%m%d', Date_of_Admission) AS INTEGER) AS date2"
    "When asked for few rows, use LIMIT. "
    "  - few: LIMIT 5\n"
    "  - some: LIMIT 7\n"
    "  - first 10: LIMIT 10\n"
